...
console.log("in method  +++++++++++++" + urlObject[uriIndex-1]);
	...
	break;
      ...
      case "dataType": 
        object[(data[k])] =  data[++k]; 
	urlObject[uriIndex-1][12] = object[(data[k])];
        break;
      case "LIST":
	tag = (data[k]);
        object[tag] = [];
...
if(i == tempArray.length)
{
    ...
    var generateURLCSV = function() {
      ...
      var callbackOnDict = function () {
	    ...
	    URLFileData += "test_template = ";
	    URLFileData += JSON.stringify(tempJSONURL,null, 2);
	    URLFileData += ";\n";	
	    URLFileData += "";	
            fs.writeFile('tp1_test_template.py', URLFileData, function (err) {
		if (err) return console.log(err);
		console.log('URL Array > tp1_test_template.py');
	    });
      }
